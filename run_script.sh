...
# 🚀 INITIAL STARTUP
# ==========================================
echo "Dashboard script initialized. Ready for local Live Server layout."
echo "-------------------------------------------------"
# ==========================================
...
EOF
        # Forces index.html to update right after you modify data
        node "$EXTRACT_TOOL"
...
EOF
    ...
    6) 
        ...
        echo "⚠️ WARNING: Wiping history will clear out old commit logs and"
        echo "reset your GitHub repository size back to zero baseline."
        read -p "Do you want to wipe ALL previous commit history? (y/n): " wipe_git
        if [ "$wipe_git" = "y" ] || [ "$wipe_git" = "Y" ]; then
            echo "📦 Clearing old Git history layers..."
            git checkout --orphan temp_branch_clean
            git add -A
            git commit -m "Initialize Fresh Automation Baseline"
            git branch -D main
            git branch -m main
            
            echo "📤 Force-pushing fresh baseline to GitHub..."
            git push origin main --force
            
            echo "🧹 Re-indexing local cache database..."
