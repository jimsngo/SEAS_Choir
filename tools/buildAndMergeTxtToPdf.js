...
// Merges all .txt files from moments/ and generates pdfs/lyrics.pdf with print-safe margins
...
async function mergeTxtToPdf() {
  ...
  for (const block of layout) {
    ...
    for (const momentName of block.moments) {
      ...
      if (!moment || !moment.lyrics) continue;
      const txtPath = path.isAbsolute(moment.lyrics) ? moment.lyrics : path.join(__dirname, '..', moment.lyrics);
      const meta = extractMetaFromTxt(txtPath);
